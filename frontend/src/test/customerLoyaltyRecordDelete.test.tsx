  ...
  return render(
    <MemoryRouter initialEntries={[`/loyalty-programs/${PROGID}/customers/${RECID}`]}>
      <AuthProvider>
        <BusinessProvider>
          <Routes>
            <Route path="/loyalty-programs/:programId/customers/:recordId" element={<CustomerLoyaltyRecordDelete />} />
...
describe("CustomerLoyaltyRecordDelete", () => {
  beforeEach(() => {
    localStorage.clear();
  ...
  it("renders the delete control", async () => {
    ...
    renderDelete();
    await waitFor(() => expect(screen.getByTestId("customer-loyalty-record-delete-submit")).toBeTruthy());
  });

  it("sends DELETE to the exact nested endpoint (not PUT)", async () => {
    await bootAuth(true);
    ...
    renderDelete();
    await waitFor(() => expect(screen.getByTestId("customer-loyalty-record-delete-submit")).toBeTruthy());
    screen.getByTestId("customer-loyalty-record-delete-submit").click();
    await waitFor(() => expect(fetchMock).toHaveBeenCalled());
    const called = fetchMock.mock.calls.some(
      (c) => String(c[0]).includes(`/api/v1/businesses/${BID}/loyalty-programs/${PROGID}/customers/${RECID}/`) && c[1]?.method === "DELETE",
    );
  ...
  it("shows a deleting state while in progress", async () => {
    ...
    await waitFor(() => expect(screen.getByTestId("customer-loyalty-record-delete-submit")).toBeTruthy());
    screen.getByTestId("customer-loyalty-record-delete-submit").click();
    await waitFor(() => expect(screen.getByTestId("customer-loyalty-record-delete-deleting")).toBeTruthy());
    resolveDelete();
  });

  it("handles 204 without JSON parsing and navigates to the records list after success", async () => {
    await bootAuth(true);
    const fetchMock = vi.fn(async (url: string, init?: RequestInit) => {
      if (String(url).includes("/auth/me/")) return new Response(JSON.stringify(userObj), { status: 200 });
      if (String(url).includes(`/api/v1/businesses/${BID}/loyalty-programs/${PROGID}/customers/${RECID}/`)) {
  ...
  it("handles 404 gracefully with an error state", async () => {
    ...
    await waitFor(() => expect(screen.getByTestId("customer-loyalty-record-delete-submit")).toBeTruthy());
    screen.getByTestId("customer-loyalty-record-delete-submit").click();
    await waitFor(() => expect(screen.getByTestId("customer-loyalty-record-delete-error")).toBeTruthy());
  });
});
