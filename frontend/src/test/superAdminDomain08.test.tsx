...
describe("Domain 08: Support Center Platform Frontend (RED)", () => {
  beforeEach(() => {
  ...
  it("1. Support tickets list renders under PlatformLayout and requests GET /api/v1/admin/support/tickets/", async () => {
    await bootAndCapture(true, { superuser: true });
    const { getByTestId } = render(
      <MemoryRouter initialEntries={["/platform-admin/support"]}>
        <AuthProvider>
          <AppRoutes />
        </AuthProvider>
      </MemoryRouter>,
    ...
    await waitFor(() =>
      expect(getByTestId("super-admin-support")).toBeTruthy(),
    );
    expect(
      capturedUrls.some((u) => u.includes("/api/v1/admin/support/tickets/ :: GET")),
    ).toBe(true);
  });

  it("2. Non-superuser is forbidden on support tickets list", async () => {
