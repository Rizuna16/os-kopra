...
describe("CustomerDelete", () => {
  ...
  it("renders the delete control", async () => {
    ...
    renderDelete();
    await waitFor(() => expect(screen.getByTestId("customer-delete-submit")).toBeTruthy());
  });

  it("sends DELETE to the exact customer endpoint (not PUT)", async () => {
    await bootAuth(true);
    ...
    renderDelete();
    await waitFor(() => expect(screen.getByTestId("customer-delete-submit")).toBeTruthy());
    screen.getByTestId("customer-delete-submit").click();
    await waitFor(() => expect(fetchMock).toHaveBeenCalled());
    const called = fetchMock.mock.calls.some(
      (c) => String(c[0]).includes(`/api/v1/businesses/${BID}/customers/${CID}/`) && c[1]?.method === "DELETE",
    );
  ...
  it("shows a deleting state while in progress", async () => {
    ...
    await waitFor(() => expect(screen.getByTestId("customer-delete-submit")).toBeTruthy());
    screen.getByTestId("customer-delete-submit").click();
    await waitFor(() => expect(screen.getByTestId("customer-delete-deleting")).toBeTruthy());
    resolveDelete();
  });

  it("handles 204 without JSON parsing and navigates to /customers after success", async () => {
    await bootAuth(true);
    const fetchMock = vi.fn(async (url: string, init?: RequestInit) => {
      if (String(url).includes("/auth/me/")) return new Response(JSON.stringify(userObj), { status: 200 });
      if (String(url).includes(`/api/v1/businesses/${BID}/customers/${CID}/`)) {
  ...
  it("handles 404 gracefully with an error state", async () => {
    ...
    await waitFor(() => expect(screen.getByTestId("customer-delete-submit")).toBeTruthy());
    screen.getByTestId("customer-delete-submit").click();
    await waitFor(() => expect(screen.getByTestId("customer-delete-error")).toBeTruthy());
  });
});
