...
describe("PurchaseOrderList", () => {
  ...
  it("requests purchase orders using the active business_id and renders the returned plain array", async () => {
    await bootAuth(true);
    const fetchMock = vi.fn(async (url: string) => {
      ...
      if (String(url).includes(`/api/v1/businesses/${BID}/purchase-orders/`)) {
        return new Response(
          JSON.stringify([
            { id: "po1", business: BID, supplier: "s1", location: "l1", status: "DRAFT", lines: [], created_at: "2024-01-01T00:00:00Z", updated_at: "2024-01-01T00:00:00Z" },
          ]),
    ...
    const calledForBiz = fetchMock.mock.calls.some((c) =>
      String(c[0]).includes(`/api/v1/businesses/${BID}/purchase-orders/`),
    );
  ...
  it("shows a loading state while the request is in flight", async () => {
    ...
    (globalThis as any).fetch = fetchMock;
    renderList(BID);
    await waitFor(() => expect(screen.getByTestId("purchase-order-list-loading")).toBeTruthy());
  ...
  it("handles an empty array with an empty state", async () => {
    ...
    (globalThis as any).fetch = fetchMock;
    renderList(BID);
    await waitFor(() => expect(screen.getByTestId("purchase-order-list-empty")).toBeTruthy());
  });
  ...
  it("renders a returned purchase order row", async () => {
    await bootAuth(true);
    const fetchMock = vi.fn(async (url: string) => {
      if (String(url).includes("/auth/me/")) {
        return new Response(JSON.stringify(userObj), { status: 200, headers: { "Content-Type": "application/json" } });
    ...
    (globalThis as any).fetch = fetchMock;
    renderList(BID);
    await waitFor(() => expect(screen.getByText("po1")).toBeTruthy());
  });

  it("reloads data and does not display previous business purchase orders after context switch", async () => {
    await bootAuth(true);
    const poByBiz: Record<string, unknown[]> = {
      [BID]: [
        { id: "poa", business: BID, supplier: "s1", location: "l1", status: "DRAFT", lines: [], created_at: "2024-01-01T00:00:00Z", updated_at: "2024-01-01T00:00:00Z" },
      ],
      [OTHER_BID]: [
        { id: "pob", business: OTHER_BID, supplier: "s2", location: "l2", status: "DRAFT", lines: [], created_at: "2024-01-01T00:00:00Z", updated_at: "2024-01-01T00:00:00Z" },
      ],
    };
    const fetchMock = vi.fn(async (url: string, init?: RequestInit) => {
      ...
      const match = String(url).match(/\/api\/v1\/businesses\/([^/]+)\/purchase-orders\//);
      const biz = match ? match[1] : BID;
      return new Response(JSON.stringify(poByBiz[biz] ?? []), {
        status: 200,
    ...
    const calledB = fetchMock.mock.calls.some((c) =>
      String(c[0]).includes(`/api/v1/businesses/${OTHER_BID}/purchase-orders/`) &&
      (c[1]?.method ?? "GET") === "GET",
    );
