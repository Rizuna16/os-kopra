      ...
      <AuthProvider>
        <BusinessProvider>
          <Routes>
            ...
            <Route path="/customers" element={<div data-testid="customer-list-nav">Navigated</div>} />
...
describe("CustomerCreate", () => {
  ...
  it("renders the create form with name, phone, email, address fields", async () => {
    ...
    expect(screen.getByTestId("customer-phone-input")).toBeTruthy();
    expect(screen.getByTestId("customer-email-input")).toBeTruthy();
    expect(screen.getByTestId("customer-address-input")).toBeTruthy();
    expect(screen.getByTestId("customer-create-submit")).toBeTruthy();
  });
  ...
  it("rejects an empty name without calling the API", async () => {
    ...
    renderCreate();
    await waitFor(() => expect(screen.getByTestId("customer-create-submit")).toBeTruthy());
    screen.getByTestId("customer-create-submit").click();
    await waitFor(() => expect(screen.getByTestId("customer-create-error")).toBeTruthy());
    const customerCalls = fetchMock.mock.calls.filter((c) =>
  ...
  it("submits a valid name and sends name, phone, email, address (empty optionals as empty strings)", async () => {
    ...
    const fetchMock = vi.fn(async (url: string, init?: RequestInit) => {
      ...
      if (String(url).includes(`/api/v1/businesses/${BID}/customers/`)) {
        ...
        expect(body.name).toBe("Customer A");
        ...
        return new Response(
          JSON.stringify({ id: "c1", business: BID, name: "Customer A", phone: "", email: "", address: "", created_at: "2024-01-01T00:00:00Z", updated_at: "2024-01-01T00:00:00Z" }),
  ...
  it("handles backend duplicate name error via errors.name", async () => {
    ...
    const fetchMock = vi.fn(async (url: string, init?: RequestInit) => {
      ...
      if (String(url).includes(`/api/v1/businesses/${BID}/customers/`)) {
        return new Response(
          JSON.stringify({ name: ["Customer with this name already exists."] }),
          { status: 400, headers: { "Content-Type": "application/json" } },
        );
      }
      return new Response("[]", { status: 200 });
  ...
  it("handles backend duplicate name error via errors.non_field_errors", async () => {
    ...
    const fetchMock = vi.fn(async (url: string, init?: RequestInit) => {
      ...
      if (String(url).includes(`/api/v1/businesses/${BID}/customers/`)) {
        return new Response(
          JSON.stringify({ non_field_errors: ["Customer with this name already exists."] }),
          { status: 400, headers: { "Content-Type": "application/json" } },
        );
      }
      return new Response("[]", { status: 200 });
  ...
  it("navigates to /customers on successful creation", async () => {
    await bootAuth(true);
    const fetchMock = vi.fn(async (url: string, init?: RequestInit) => {
      if (String(url).includes("/auth/me/")) return new Response(JSON.stringify(userObj), { status: 200 });
      if (String(url).includes(`/api/v1/businesses/${BID}/customers/`)) {
    ...
    nameInput.value = "Customer A";
    screen.getByTestId("customer-create-submit").click();
    await waitFor(() => expect(screen.getByTestId("customer-list-nav")).toBeTruthy());
  });
});
