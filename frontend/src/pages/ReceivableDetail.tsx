...
import type { Receivable, PaymentMethodChoice } from "../receivable/types";
...
export function ReceivableDetail() {
  ...
  const loadData = () => {
    if (!currentBusinessId || !receivableId) return;
    setLoading(true);
    setError(null);
    getReceivable(currentBusinessId, receivableId, currentLocationId || undefined)
      .then((data) => {
      ...
      .catch((e) => {
        ...
        const msg = e instanceof ApiError ? e.message : "Failed to load receivable detail.";
        setError(msg);
      })
  ...
  }, [currentBusinessId, receivableId, currentLocationId]);

  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 p-6 flex items-center justify-center">
        <div data-testid="detail-loading" className="text-gray-500 font-medium">Memuat detail piutang…</div>
      </div>
    );
  }
  ...
  const handlePaySubmit = async (e: React.FormEvent) => {
    ...
    try {
      await payReceivable(currentBusinessId, receivableId, {
        amount: payAmount,
  ...
  const handleReverseSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!currentBusinessId || !receivableId || !targetPaymentId) return;
    setRevError(null);
    ...
    try {
      await reversePayment(currentBusinessId, receivableId, targetPaymentId, {
  ...
  const handleCloseSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await closeReceivable(currentBusinessId, receivableId, { notes: closeNotes });
    ...
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
        <div className="space-y-1">
          ...
          <p className="text-sm text-gray-500">ID Piutang: {item.id}</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {!isTerminal && isOwner && (
            <button
              ...
              data-testid="btn-close-receivable"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-1">
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Sisa Tagihan</p>
      ...
      <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4" data-testid="detail-payment-history">
        <h2 className="text-lg font-bold text-gray-900">Riwayat Pembayaran (Payment Allocations)</h2>
      ...
      {showPayModal && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-xl max-w-md w-full p-6 space-y-4">
            <h3 className="text-lg font-bold text-gray-900">Catat Pembayaran Piutang</h3>

            <div className="bg-blue-50 p-3 rounded-xl border border-blue-100 text-sm text-blue-900">
              Sisa Tagihan Saat Ini: <strong data-testid="modal-pay-remaining-outstanding">{formatRupiah(item.outstanding_amount)}</strong>
            </div>
      ...
      {showReverseModal && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-xl max-w-md w-full p-6 space-y-4">
            ...
            <p className="text-xs text-gray-600">Tindakan ini akan mengembalikan saldo outstanding piutang ini. Owner Only.</p>
      ...
      {showCloseModal && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-xl max-w-md w-full p-6 space-y-4">
            <h3 className="text-lg font-bold text-gray-900">Tutup Piutang (Write-Off)</h3>
            <div className="bg-amber-50 p-3 rounded-xl border border-amber-200 text-xs text-amber-900">
              <strong>PENTING:</strong> Menutup piutang akan menghapus sisa tagihan (set to 0,00) sebagai penghapusan piutang (CLOSED != PAID).
            </div>
            ...
            <form onSubmit={handleCloseSubmit} className="space-y-4">
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                ...
                >
                  {closeSubmitting ? "Memproses…" : "Konfirmasi Tutup Piutang"}
                </button>
              </div>
            </form>
          </div>
