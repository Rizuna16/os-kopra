...
import { getEmployee, updateEmployee } from "../employee/employeeService";
import type { Employee, EmployeeUpdatePayload } from "../employee/types";
...
export function EmployeeEdit() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateEmployee(currentBusinessId, employeeId, payload);
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-6">
            <h1 className="text-2xl font-bold tracking-tight text-gray-900 mb-4">Edit employee</h1>
            <div data-testid="employee-edit-loading" className="text-sm text-gray-500">Loading…</div>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-6">
            <h1 className="text-2xl font-bold tracking-tight text-gray-900 mb-4">Edit employee</h1>
            <div data-testid="employee-edit-error" className="text-sm text-red-600 bg-red-50 border border-red-100 rounded-xl p-3 sm:p-4">{error}</div>
          </div>
        </div>
      </div>
    );
  ...
  if (!item) return <div data-testid="employee-edit">Employee not found.</div>;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-6">
          <h1 className="text-2xl font-bold tracking-tight text-gray-900 mb-4">Edit employee</h1>
          <form data-testid="employee-edit-form" onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label htmlFor="name" className="block text-sm font-medium text-gray-700 mb-1">Name</label>
              <input
                id="name"
                type="text"
                data-testid="employee-edit-name-input"
                ref={nameRef}
            ...
            <div>
              ...
              <input
                id="code"
                type="text"
                data-testid="employee-edit-code-input"
                ref={codeRef}
            ...
            <div>
              ...
              <input
                id="hire_date"
                type="date"
                data-testid="employee-edit-hire-date-input"
                ref={hireDateRef}
                defaultValue={item.hire_date ?? ""}
                className="w-full px-4 py-2.5 text-sm rounded-xl border border-gray-300 bg-white text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent transition-all"
              />
            </div>
            <div className="flex items-center">
              <input
                id="active"
                type="checkbox"
                data-testid="employee-edit-active-input"
                ref={activeRef}
            ...
            {error && (
              <div data-testid="employee-edit-error" className="text-sm text-red-600 bg-red-50 border border-red-100 rounded-xl p-3 sm:p-4">
                {error}
              </div>
            )}
            <button
              type="submit"
              data-testid="employee-edit-submit"
              disabled={loading}
