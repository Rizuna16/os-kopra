...
def user(db):
    return User.objects.create_user(
        email="employee_owner@example.com",
...
def business(db, user):
    return Business.objects.create(name="Toko Employee", owner=user)


@pytest.fixture
def other_business(db, other_user):
    return Business.objects.create(name="Toko Lain Employee", owner=other_user)
