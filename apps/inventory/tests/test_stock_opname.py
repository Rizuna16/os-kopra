...
class TestStockOpname:
    ...
    def test_physical_lower_sets_quantity(self, auth_client, location, variant, stock):
        response = _post(
            auth_client,
            {
                "location": str(location.id),
        ...
        assert response.data["quantity"] == "7.00"

    def test_physical_higher_sets_quantity(self, auth_client, location, variant, stock):
        response = _post(
            auth_client,
            {
                "location": str(location.id),
    ...
    def test_physical_same_keeps_quantity(self, auth_client, location, variant, stock):
        response = _post(
            auth_client,
            {
                "location": str(location.id),
    ...
    def test_physical_zero_sets_quantity_zero(self, auth_client, location, variant, stock):
        response = _post(
            auth_client,
            {
                "location": str(location.id),
    ...
    def test_positive_on_missing_stock_creates(self, auth_client, location, variant):
        ...
        assert new.quantity == Decimal("8")

    def test_zero_on_missing_stock_not_created(self, auth_client, location, variant):
        assert not Stock.objects.filter(location=location, variant=variant).exists()
        response = _post(
            auth_client,
            {
    ...
    def test_negative_quantity_rejected(self, auth_client, location, variant, stock):
        response = _post(
            auth_client,
            {
                "location": str(location.id),
    ...
    def test_invalid_quantity_rejected(self, auth_client, location, variant, stock):
        response = _post(
            auth_client,
            {
                "location": str(location.id),
    ...
    def test_cross_business_location_rejected(self, auth_client, other_location, variant, stock):
        response = _post(
            auth_client,
            {
    ...
    def test_cross_business_variant_rejected(self, auth_client, location, other_variant, stock):
        response = _post(
            auth_client,
            {
                "location": str(location.id),
    ...
    def test_location_variant_mismatch_rejected(self, auth_client, other_location, other_variant):
        response = _post(
            auth_client,
            {
                "location": str(other_location.id),
    ...
    def test_other_user_cannot_opname_own_business(self, other_auth_client, location, variant, stock):
        response = _post(
            other_auth_client,
            {
                "location": str(location.id),
    ...
    def test_product_and_variant_unchanged(self, auth_client, location, variant, product, stock):
        _post(
            auth_client,
            {
                "location": str(location.id),
